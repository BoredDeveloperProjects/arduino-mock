...
public:
  ...
  // virtual bool findUntil(uint8_t *target, char *terminator) { return
  // findUntil((char *)target, terminator) = 0; }
  //
  // virtual bool findUntil(char *target, size_t targetLen, char *terminate,
  // size_t termLen) = 0; virtual bool findUntil(uint8_t *target, size_t
  // targetLen, char *terminate, size_t termLen) {return findUntil((char
  // *)target, targetLen, terminate, termLen) = 0; }
  //
  ...
  // virtual size_t readBytesUntil( char terminator, char *buffer, size_t
  // length); virtual size_t readBytesUntil( char terminator, uint8_t *buffer,
  // size_t length) { return readBytesUntil(terminator, (char *)buffer, length);
  // }
  //
...
public:
  ...
  // virtual bool findUntil(uint8_t *target, char *terminator) { return
  // findUntil((char *)target, terminator); }
  //
  // virtual bool findUntil(char *target, size_t targetLen, char *terminate,
  // size_t termLen); virtual bool findUntil(uint8_t *target, size_t targetLen,
  // char *terminate, size_t termLen) {return findUntil((char *)target,
  // targetLen, terminate, termLen); }
  //
