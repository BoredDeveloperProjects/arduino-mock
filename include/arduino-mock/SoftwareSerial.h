...
/**
  ...
  \note In the actual Arduino architecture, SoftwareSerial does not inherit from
  HardwareSerial but from Stream, meaning that SoftwareSerial and HardwareSerial
  are rather siblings. In the mock we define SoftwareSerial as a child of
  HardwareSerial for the sake of simplicity and to avoid conflicts when
...
/**
  ...
  The class implements an internal buffer, where the user can pre-load the data
  that the fake will serve on subsequent calls to related methods: available,
  read and operator [].

  Limitations:
  - Buffer size is fixed, see private const buffer_size
  - It is not a ringbuffer: subsequent calls to buffer_load will override
...
/**
  ...
  \brief Standard GoogleMock class of SoftwareSerial. It uses a private instance
  of SoftwareSerialFake to mock the behaviour of RX related methods available,
  read and operator []
*/
