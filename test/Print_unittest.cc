...
static void emitLog(Print *output, const char *board, const char *component, const char *level,
                    const char *message) {
  output->print("[");
  output->print(board);
  if (component) {
...
TEST_F(PrintTest, forwardsPrintOverloads) {
  ...
  EXPECT_CALL(*printMock, print(Matcher<const char *>(StrEq("hello")))).WillOnce(Return(5));
  EXPECT_EQ(5, output.print("hello"));

  EXPECT_CALL(*printMock, print(Matcher<char>('x'))).WillOnce(Return(1));
  EXPECT_EQ(1, output.print('x'));

  EXPECT_CALL(*printMock, print(Matcher<int>(42), Matcher<int>(DEC))).WillOnce(Return(2));
  EXPECT_EQ(2, output.print(42));

  EXPECT_CALL(*printMock, print(Matcher<int>(255), Matcher<int>(HEX))).WillOnce(Return(2));
  EXPECT_EQ(2, output.print(255, HEX));

  EXPECT_CALL(*printMock, print(Matcher<double>(3.14), Matcher<int>(2))).WillOnce(Return(4));
...
TEST_F(PrintTest, forwardsPrintlnOverloads) {
  ...
  EXPECT_CALL(*printMock, println(Matcher<const char *>(StrEq("hello")))).WillOnce(Return(6));
  ...
  EXPECT_CALL(*printMock, println(Matcher<int>(42), Matcher<int>(DEC))).WillOnce(Return(3));
  EXPECT_EQ(3, output.println(42));

  EXPECT_CALL(*printMock, println(Matcher<double>(3.14), Matcher<int>(2))).WillOnce(Return(5));
...
TEST_F(PrintTest, serialCanBeUsedAsPrintTarget) {
  ...
  EXPECT_CALL(*serialMock, print(Matcher<const char *>(StrEq("serial")))).WillOnce(Return(6));
...
TEST_F(PrintTest, realisticStructuredLogUsage) {
  ...
  EXPECT_CALL(*printMock, print(Matcher<const char *>(StrEq("] "))));
  EXPECT_CALL(*printMock, println(Matcher<const char *>(StrEq("system ready"))));
